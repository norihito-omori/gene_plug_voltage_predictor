...
保持値汚染が残っていた（EP370G 8950 で停止中行の 99.4% が保持値 > 0 と判明）。
全ての電圧系統計（`voltage_active_rate`, `exchange_candidates_*`,
`plug_activity_imbalance_ratio`）は運転中行のみで計算。

累積運転時間はパック形式（上位 3 バイト=時 / 下位 1 バイト=分）をアンパックして
実時間（h）に変換してから統計を取る（`rt_min` / `rt_max` / `rt_span` は全て単位「時」）。
...
def _analyze_one(path: Path) -> LocationStats:
    ...
    rt_all_zero = bool((rt == 0.0).all())

    voltage_df = df[list(voltage_cols)]
    if rows:
        voltage_active_rate = float(running_mask.mean())
    ...
    for col in voltage_cols:
        col_values = voltage_df[col]
        # 運転中かつ当該プラグのセンサー値が有効（非 NaN かつ > 0）な行を「active」とする。
        # 停止中保持値は running_mask で除外される。プラグ毎にセンサー欠測パターンが違う
        # ことを捉えるため、running_mask.sum() ではなく per-plug で数える。
        plug_active_mask = running_mask & col_values.notna() & (col_values > 0)
        active_rows.append(int(plug_active_mask.sum()))
        # 急変検出は運転中行だけで計算（停止中保持値の前後で誤検出しないため）
        col_series = col_values.where(running_mask)
        diff = col_series.diff().abs()
...
def _write_markdown(df: pd.DataFrame, out_path: Path, model_label: str, input_dir: Path) -> None:
    header = (
        ...
        f"- 入力: `{input_dir}`\n"
        f"- 件数: {len(df)} 機場\n"
        f"- ソート: `rt_span` 降順（累積運転時間の伸びが大きい順、単位: 時）\n"
        f"- `rt_min` / `rt_max` / `rt_span`: 単位は「時」（パック形式をアンパック済み）\n"
        f"- 運転中判定: `発電機電力 > 0`（ADR-013 C-1 停止中保持値問題への対応）\n"
        f"- 交換候補しきい値: 要求電圧の 1 ステップ絶対値変化 > {_EXCHANGE_JUMP_THRESHOLD}（運転中行のみ）\n"
        f"- `voltage_active_rate`: 全行中で `発電機電力 > 0` の行率\n"
        f"- `active_rows_per_plug`: プラグ毎の「運転中 ∧ 要求電圧 > 0」行数（保持値除去後のセンサー有効行数）\n"
